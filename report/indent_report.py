...
class indent_report(osv.osv):
    ...
    _columns = {
        ...
        'order_type':fields.selection([('factory','Factory'),('store','Store')],'Order Type',readonly=True),
    ...
    def init(self, cr):
        tools.sql.drop_view_if_exists(cr, 'indent_report')
        cr.execute("""
            create or replace view indent_report as (
                ...
                from indent_order_line l
                    join indent_order s on (l.order_id=s.id)
